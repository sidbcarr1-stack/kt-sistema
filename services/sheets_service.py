...
class SheetsService:
    ...
    def excluir_registro(self, id_valor):
        """Remove a linha inteira da planilha."""
        try:
            aba = self._obter_aba()
            linha_planilha = self._encontrar_linha_por_id(aba, id_valor)
    ...
    def _obter_aba_por_nome(self, nome_aba):
        """Retorna uma aba específica da planilha pelo nome."""
        try:
            return self.ss.worksheet(nome_aba)
